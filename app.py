...
for entry in watchlist:
    ...
    ltp, pct_change = get_live_price(scripcode) if scripcode else (None, None)
...
FULL_UNIVERSE = [
    ...
    "JIOFIN", "TATASTEEL", "E2E", "GOPAL", "SANDUMA", "HINDZINC", "SAGILITY",
...
def fetch_one(symbol):
    ...
    if not scripcode:
        return None
    ltp, pct_change = get_live_price(scripcode)
...
if not gainers_df.empty:
    st.dataframe(
        gainers_df.style.format({"price": "Rs.{:.2f}", "change": "{:+.2f}%"}),
...
if not losers_df.empty:
    st.dataframe(
        losers_df.style.format({"price": "Rs.{:.2f}", "change": "{:+.2f}%"}),
        width="stretch", hide_index=True
    )